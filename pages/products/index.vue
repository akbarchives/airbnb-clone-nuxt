<template>
  <NuxtLayout :name="layout">
    <div v-if="pending">
      <!-- skeleron card load -->
      <div
        class="custom-container mx-auto mb-36 grid max-w-screen-2xl animate-pulse grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4"
      >
        <div class="flex flex-col overflow-hidden">
          <div
            class="flex h-48 items-center justify-center rounded-xl bg-gray-400"
          >
            <svg
              class="h-12 w-12 self-center text-gray-200"
              xmlns="http://www.w3.org/2000/svg"
              aria-hidden="true"
              fill="currentColor"
              viewBox="0 0 640 512"
            >
              <path
                d="M480 80C480 35.82 515.8 0 560 0C604.2 0 640 35.82 640 80C640 124.2 604.2 160 560 160C515.8 160 480 124.2 480 80zM0 456.1C0 445.6 2.964 435.3 8.551 426.4L225.3 81.01C231.9 70.42 243.5 64 256 64C268.5 64 280.1 70.42 286.8 81.01L412.7 281.7L460.9 202.7C464.1 196.1 472.2 192 480 192C487.8 192 495 196.1 499.1 202.7L631.1 419.1C636.9 428.6 640 439.7 640 450.9C640 484.6 612.6 512 578.9 512H55.91C25.03 512 .0006 486.1 .0006 456.1L0 456.1z"
              />
            </svg>
          </div>
          <div class="mt-2 h-4 w-8/12 rounded-full bg-gray-400"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
          <div class="white mt-2 h-2 w-6/12"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
        </div>
        <div class="flex flex-col overflow-hidden">
          <div
            class="flex h-48 items-center justify-center rounded-xl bg-gray-400"
          >
            <svg
              class="h-12 w-12 self-center text-gray-200"
              xmlns="http://www.w3.org/2000/svg"
              aria-hidden="true"
              fill="currentColor"
              viewBox="0 0 640 512"
            >
              <path
                d="M480 80C480 35.82 515.8 0 560 0C604.2 0 640 35.82 640 80C640 124.2 604.2 160 560 160C515.8 160 480 124.2 480 80zM0 456.1C0 445.6 2.964 435.3 8.551 426.4L225.3 81.01C231.9 70.42 243.5 64 256 64C268.5 64 280.1 70.42 286.8 81.01L412.7 281.7L460.9 202.7C464.1 196.1 472.2 192 480 192C487.8 192 495 196.1 499.1 202.7L631.1 419.1C636.9 428.6 640 439.7 640 450.9C640 484.6 612.6 512 578.9 512H55.91C25.03 512 .0006 486.1 .0006 456.1L0 456.1z"
              />
            </svg>
          </div>
          <div class="mt-2 h-4 w-8/12 rounded-full bg-gray-400"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
          <div class="white mt-2 h-2 w-6/12"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
        </div>
        <div class="flex flex-col overflow-hidden">
          <div
            class="flex h-48 items-center justify-center rounded-xl bg-gray-400"
          >
            <svg
              class="h-12 w-12 self-center text-gray-200"
              xmlns="http://www.w3.org/2000/svg"
              aria-hidden="true"
              fill="currentColor"
              viewBox="0 0 640 512"
            >
              <path
                d="M480 80C480 35.82 515.8 0 560 0C604.2 0 640 35.82 640 80C640 124.2 604.2 160 560 160C515.8 160 480 124.2 480 80zM0 456.1C0 445.6 2.964 435.3 8.551 426.4L225.3 81.01C231.9 70.42 243.5 64 256 64C268.5 64 280.1 70.42 286.8 81.01L412.7 281.7L460.9 202.7C464.1 196.1 472.2 192 480 192C487.8 192 495 196.1 499.1 202.7L631.1 419.1C636.9 428.6 640 439.7 640 450.9C640 484.6 612.6 512 578.9 512H55.91C25.03 512 .0006 486.1 .0006 456.1L0 456.1z"
              />
            </svg>
          </div>
          <div class="mt-2 h-4 w-8/12 rounded-full bg-gray-400"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
          <div class="white mt-2 h-2 w-6/12"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
        </div>
        <div class="flex flex-col overflow-hidden">
          <div
            class="flex h-48 items-center justify-center rounded-xl bg-gray-400"
          >
            <svg
              class="h-12 w-12 self-center text-gray-200"
              xmlns="http://www.w3.org/2000/svg"
              aria-hidden="true"
              fill="currentColor"
              viewBox="0 0 640 512"
            >
              <path
                d="M480 80C480 35.82 515.8 0 560 0C604.2 0 640 35.82 640 80C640 124.2 604.2 160 560 160C515.8 160 480 124.2 480 80zM0 456.1C0 445.6 2.964 435.3 8.551 426.4L225.3 81.01C231.9 70.42 243.5 64 256 64C268.5 64 280.1 70.42 286.8 81.01L412.7 281.7L460.9 202.7C464.1 196.1 472.2 192 480 192C487.8 192 495 196.1 499.1 202.7L631.1 419.1C636.9 428.6 640 439.7 640 450.9C640 484.6 612.6 512 578.9 512H55.91C25.03 512 .0006 486.1 .0006 456.1L0 456.1z"
              />
            </svg>
          </div>
          <div class="mt-2 h-4 w-8/12 rounded-full bg-gray-400"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
          <div class="white mt-2 h-2 w-6/12"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
        </div>
        <div class="flex flex-col overflow-hidden">
          <div
            class="flex h-48 items-center justify-center rounded-xl bg-gray-400"
          >
            <svg
              class="h-12 w-12 self-center text-gray-200"
              xmlns="http://www.w3.org/2000/svg"
              aria-hidden="true"
              fill="currentColor"
              viewBox="0 0 640 512"
            >
              <path
                d="M480 80C480 35.82 515.8 0 560 0C604.2 0 640 35.82 640 80C640 124.2 604.2 160 560 160C515.8 160 480 124.2 480 80zM0 456.1C0 445.6 2.964 435.3 8.551 426.4L225.3 81.01C231.9 70.42 243.5 64 256 64C268.5 64 280.1 70.42 286.8 81.01L412.7 281.7L460.9 202.7C464.1 196.1 472.2 192 480 192C487.8 192 495 196.1 499.1 202.7L631.1 419.1C636.9 428.6 640 439.7 640 450.9C640 484.6 612.6 512 578.9 512H55.91C25.03 512 .0006 486.1 .0006 456.1L0 456.1z"
              />
            </svg>
          </div>
          <div class="mt-2 h-4 w-8/12 rounded-full bg-gray-400"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
          <div class="white mt-2 h-2 w-6/12"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
        </div>
        <div class="flex flex-col overflow-hidden">
          <div
            class="flex h-48 items-center justify-center rounded-xl bg-gray-400"
          >
            <svg
              class="h-12 w-12 self-center text-gray-200"
              xmlns="http://www.w3.org/2000/svg"
              aria-hidden="true"
              fill="currentColor"
              viewBox="0 0 640 512"
            >
              <path
                d="M480 80C480 35.82 515.8 0 560 0C604.2 0 640 35.82 640 80C640 124.2 604.2 160 560 160C515.8 160 480 124.2 480 80zM0 456.1C0 445.6 2.964 435.3 8.551 426.4L225.3 81.01C231.9 70.42 243.5 64 256 64C268.5 64 280.1 70.42 286.8 81.01L412.7 281.7L460.9 202.7C464.1 196.1 472.2 192 480 192C487.8 192 495 196.1 499.1 202.7L631.1 419.1C636.9 428.6 640 439.7 640 450.9C640 484.6 612.6 512 578.9 512H55.91C25.03 512 .0006 486.1 .0006 456.1L0 456.1z"
              />
            </svg>
          </div>
          <div class="mt-2 h-4 w-8/12 rounded-full bg-gray-400"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
          <div class="white mt-2 h-2 w-6/12"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
        </div>
        <div class="flex flex-col overflow-hidden">
          <div
            class="flex h-48 items-center justify-center rounded-xl bg-gray-400"
          >
            <svg
              class="h-12 w-12 self-center text-gray-200"
              xmlns="http://www.w3.org/2000/svg"
              aria-hidden="true"
              fill="currentColor"
              viewBox="0 0 640 512"
            >
              <path
                d="M480 80C480 35.82 515.8 0 560 0C604.2 0 640 35.82 640 80C640 124.2 604.2 160 560 160C515.8 160 480 124.2 480 80zM0 456.1C0 445.6 2.964 435.3 8.551 426.4L225.3 81.01C231.9 70.42 243.5 64 256 64C268.5 64 280.1 70.42 286.8 81.01L412.7 281.7L460.9 202.7C464.1 196.1 472.2 192 480 192C487.8 192 495 196.1 499.1 202.7L631.1 419.1C636.9 428.6 640 439.7 640 450.9C640 484.6 612.6 512 578.9 512H55.91C25.03 512 .0006 486.1 .0006 456.1L0 456.1z"
              />
            </svg>
          </div>
          <div class="mt-2 h-4 w-8/12 rounded-full bg-gray-400"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
          <div class="white mt-2 h-2 w-6/12"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
        </div>
        <div class="flex flex-col overflow-hidden">
          <div
            class="flex h-48 items-center justify-center rounded-xl bg-gray-400"
          >
            <svg
              class="h-12 w-12 self-center text-gray-200"
              xmlns="http://www.w3.org/2000/svg"
              aria-hidden="true"
              fill="currentColor"
              viewBox="0 0 640 512"
            >
              <path
                d="M480 80C480 35.82 515.8 0 560 0C604.2 0 640 35.82 640 80C640 124.2 604.2 160 560 160C515.8 160 480 124.2 480 80zM0 456.1C0 445.6 2.964 435.3 8.551 426.4L225.3 81.01C231.9 70.42 243.5 64 256 64C268.5 64 280.1 70.42 286.8 81.01L412.7 281.7L460.9 202.7C464.1 196.1 472.2 192 480 192C487.8 192 495 196.1 499.1 202.7L631.1 419.1C636.9 428.6 640 439.7 640 450.9C640 484.6 612.6 512 578.9 512H55.91C25.03 512 .0006 486.1 .0006 456.1L0 456.1z"
              />
            </svg>
          </div>
          <div class="mt-2 h-4 w-8/12 rounded-full bg-gray-400"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
          <div class="white mt-2 h-2 w-6/12"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
        </div>
        <div class="flex flex-col overflow-hidden">
          <div
            class="flex h-48 items-center justify-center rounded-xl bg-gray-400"
          >
            <svg
              class="h-12 w-12 self-center text-gray-200"
              xmlns="http://www.w3.org/2000/svg"
              aria-hidden="true"
              fill="currentColor"
              viewBox="0 0 640 512"
            >
              <path
                d="M480 80C480 35.82 515.8 0 560 0C604.2 0 640 35.82 640 80C640 124.2 604.2 160 560 160C515.8 160 480 124.2 480 80zM0 456.1C0 445.6 2.964 435.3 8.551 426.4L225.3 81.01C231.9 70.42 243.5 64 256 64C268.5 64 280.1 70.42 286.8 81.01L412.7 281.7L460.9 202.7C464.1 196.1 472.2 192 480 192C487.8 192 495 196.1 499.1 202.7L631.1 419.1C636.9 428.6 640 439.7 640 450.9C640 484.6 612.6 512 578.9 512H55.91C25.03 512 .0006 486.1 .0006 456.1L0 456.1z"
              />
            </svg>
          </div>
          <div class="mt-2 h-4 w-8/12 rounded-full bg-gray-400"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
          <div class="white mt-2 h-2 w-6/12"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
        </div>
        <div class="flex flex-col overflow-hidden">
          <div
            class="flex h-48 items-center justify-center rounded-xl bg-gray-400"
          >
            <svg
              class="h-12 w-12 self-center text-gray-200"
              xmlns="http://www.w3.org/2000/svg"
              aria-hidden="true"
              fill="currentColor"
              viewBox="0 0 640 512"
            >
              <path
                d="M480 80C480 35.82 515.8 0 560 0C604.2 0 640 35.82 640 80C640 124.2 604.2 160 560 160C515.8 160 480 124.2 480 80zM0 456.1C0 445.6 2.964 435.3 8.551 426.4L225.3 81.01C231.9 70.42 243.5 64 256 64C268.5 64 280.1 70.42 286.8 81.01L412.7 281.7L460.9 202.7C464.1 196.1 472.2 192 480 192C487.8 192 495 196.1 499.1 202.7L631.1 419.1C636.9 428.6 640 439.7 640 450.9C640 484.6 612.6 512 578.9 512H55.91C25.03 512 .0006 486.1 .0006 456.1L0 456.1z"
              />
            </svg>
          </div>
          <div class="mt-2 h-4 w-8/12 rounded-full bg-gray-400"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
          <div class="white mt-2 h-2 w-6/12"></div>
          <div class="mt-2 h-4 w-6/12 rounded-full bg-gray-400"></div>
        </div>
      </div>
    </div>
    <div v-else>
      <div
        class="custom-container mx-auto mb-36 grid max-w-screen-2xl grid-cols-1 gap-4 px-6 pt-6 sm:grid-cols-2 sm:px-8 md:grid-cols-3 lg:grid-cols-4"
      >
        <NuxtLink
          class=""
          v-for="p in products2.products"
          :to="`/products/${p.id}`"
          :key="p.id"
        >
          <CardSwipe
            :title="p.title"
            :rating="p.rating"
            :category="p.category"
            :description="p.description"
            :price="p.price"
            :brand="p.brand"
            :thumbnail="p.thumbnail"
            :img1="p.images[0]"
            :img2="p.images[1]"
          />
        </NuxtLink>
      </div>
    </div>
    <!--  -->
  </NuxtLayout>

  <!-- <div v-for="p in produk">
      {{ p.id }}
    </div> -->
  <!-- <div v-for="p in products2.products">
      {{ p.title }}
  
      <img
        :src="p.images[0]"
        alt=""
      />
    </div> -->

  <!-- <NuxtLayout :name="layout">
      <div
        class="custom-container mx-auto mb-36 grid max-w-screen-2xl grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4"
      >
        <Card
          v-for="p in products2.products"
          :title="p.title"
          :description="p.description"
          :category="p.category"
          :price="p.price"
          :image="p.images[0]"
        />
      </div>
    </NuxtLayout> -->
</template>

<script setup>
const layout = 'custom';

const { pending, data: products2 } = useFetch(
  'https://dummyjson.com/products',
  { lazy: true }
);
console.log(products2);

// const getProducts = async () => {
//   const product = await axios.get('https://dummyjson.com/products');
//   // console.log(product.data.results);
//   return product.data.products;
// };
// const produk = getProducts();
// console.log(produk);
// console.log(getProducts());
// getProducts();

const limitationList = 5;
</script>
